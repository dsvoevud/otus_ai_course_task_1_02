...
@router.get(
    ...
    description="Retrieve statistics about submitted questionnaires",
    tags=["Statistics"]
)
async def get_statistics() -> dict:
    """Get statistics about submitted questionnaires."""
    if answer_service is None:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Service not initialized"
    ...
    try:
        count = answer_service.get_submitted_count()
        return {"total_submitted": count}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
